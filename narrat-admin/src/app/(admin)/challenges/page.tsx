...
import { CHALLENGE_CATEGORY_LABELS, timeAgo } from "@/lib/utils";
...
export default function ChallengesPage() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get("/admin/challenges", { params: { page, pageSize: 20, search: search || undefined } });
      setData(res.data);
    } catch (err) {
  ...
  const columns: Column<Challenge>[] = [
    {
      ...
      cell: (row) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">{CHALLENGE_CATEGORY_LABELS[row.category]} · {row.durationDays} jours</p>
        </div>
      ),
    },
    {
      ...
      header: "Intensité",
      cell: (row) => (
        <Badge variant={INTENSITY_VARIANTS[row.intensity] ?? "default"}>
          {INTENSITY_LABELS[row.intensity] ?? row.intensity}
        </Badge>
      ),
    },
    {
    ...
    },
    {
      key: "participants",
      header: "Participants",
      cell: (row) => <span className="text-sm font-mono">{row.participantCount}</span>,
    },
    {
      key: "actions",
      header: "",
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Button size="sm"><Plus className="w-3.5 h-3.5" />Nouveau défi</Button>
      </div>
      ...
      {viewMode === "list" ? (
        <DataTable
          ...
          data={challenges}
          loading={loading}
          emptyMessage="Aucun défi trouvé"
          page={page}
